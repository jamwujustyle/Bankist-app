...
console.log(
  `Sarah's dog eating too ${
    sarahDog.curFood > sarahDog.recommendedPortion ? 'much' : 'little'
  }`
);

const ownersEatTooMuch = dogs
  .filter(dog => dog.curFood > dog.recommendedPortion * 0.9)
  .flatMap(dog => dog.owners);
const ownersEatTooLittle = dogs
  .filter(dog => dog.curFood < dog.recommendedPortion * 1.1)
  .flatMap(dog => dog.owners);

console.log();

const tooMuchString = `${ownersEatTooMuch.join(' and ')}'s eat too much`;
const tooLittleString = `${ownersEatTooLittle.join(' and ')}'s eat too little`;
